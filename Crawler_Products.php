...
final class Crawler_Products {
  ...
  private static array $source_urls = array(
  ...
  private array $products_new = array();
  ...
  public function __construct()
  {
    ...
    add_action( 'init', array( $this, 'db_add_table_crawler_products' ) );

    // AJAX crawler products on button click in admin
    add_action( 'wp_ajax_scan_crawler_products', array( $this, 'scan_on_ajax' ) );

    if ( ! wp_next_scheduled( $this->cron_hook, array( $this->cron_id ) ) ) {
      $timestamp = strtotime( 'tomorrow 3 am' ); // by UTC it will be at morning 1 am. Examples: tomorrow 2 am / today 5:15 am
      wp_schedule_event( $timestamp, 'daily', $this->cron_hook, array( $this->cron_id ) );
    }
    add_action( $this->cron_hook, array( $this, 'cron_handle_crawler_products' ) );
  }
  ...
  {
    $url = add_query_arg( array( 'limit' => $limit, 'page' => $page ), $target_url );
  ...
  {
    $products_to_update = $this->products_to_update[ $source_url ] ?? array();
    $products_new = $this->products_new[ $source_url ] ?? array();
    $none = 'No changes';
    $message = '';
